            ...
            <div className='mt-4 grid grid-cols-2 gap-x-4 gap-y-4 md:gap-x-8 md:gap-y-6'>
                {developer.skills.map(skill => (
                    <div key={skill.id} className='flex flex-col gap-1'>
                        <div className='flex gap-2 items-center'>
                            <div className='p-0.5 sm:p-1 w-5 h-5 sm:w-6 sm:h-6 md:p-2 md:w-10 md:h-10 bg-orange-500 rounded-sm'>
                                <ImageLoader loadingText={false} src={skill.icon} className='w-full h-full rounded-sm object-cover' />
                            </div>
